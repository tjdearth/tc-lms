        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {quickActions.map((action) => (
            <Link
              key={action.label}
              href={action.href}
              className="bg-white rounded-xl border border-gray-200 border-l-[3px] border-l-[#27a28c] p-4 flex items-center gap-3 hover:shadow-sm hover:border-gray-300 hover:border-l-[#1e7a6a] transition-all group"
            >
              <div className="w-10 h-10 rounded-lg bg-[#27a28c]/10 flex items-center justify-center group-hover:bg-[#27a28c]/20 transition-colors">
                {action.icon}
              </div>
              <span className="text-sm font-medium text-[#304256] group-hover:text-navy transition-colors">{action.label}</span>
            </Link>
          ))}
        </div>
